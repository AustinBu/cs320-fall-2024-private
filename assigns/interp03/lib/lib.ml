...
let type_of' ctxt expr =
  let rec loop ctxt expr =
    ...
    | OptMatch { matched; some_name; some_case; none_case } -> 
      ...
      let ctxt_cons = Env.add some_name ([], alpha) ctxt in
    ...
    | ListMatch { matched; hd_name; tl_name; cons_case; nil_case } -> 
      let tm, cm = loop ctxt matched in
      let alpha = TVar (gensym ()) in
      let ctxt_cons = Env.add hd_name ([], alpha) (Env.add tl_name ([], TList alpha) ctxt) in
    ...
    | Let { is_rec; name; value; body } ->
      if is_rec then
        let alpha = TVar (gensym ()) in
        let beta = TVar (gensym ()) in
        let ctxt' = Env.add name ([], TFun (alpha, beta)) ctxt in
        let tv, cv = loop ctxt' value in
        let ctxt'' = Env.add name ([], tv) ctxt in
        let tb, cb = loop ctxt'' body in
        (tb, (tv, TFun (alpha, beta)) :: cv @ cb)
      else 
        let tv, cv = loop ctxt value in
        let tb, cb = loop (Env.add name ([], tv) ctxt) body in
        (tb, cv @ cb)
  in loop ctxt expr
